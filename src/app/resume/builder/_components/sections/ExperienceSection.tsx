...
export function ExperienceSection({ experience, templateId }: WorkExperienceSectionProps) {
            ...
            {job.achievements && job.achievements.length > 0 && (
              <ul className={classes.experience.achievementList}>
                {job.achievements.map((achievement, achIndex) => (
                  <li key={achIndex} className={classes.experience.achievement}>{achievement}</li>
                ))}
              </ul>
            )}
          </div>
