...
export function ResumeHeader({ profile, templateId }: ResumeHeaderProps) {
  const classes = getThemeClasses(templateId);
	...
	return (
		<>
			{profile && (
				<div className={classes.header.wrapper}> {/* modern: header.containerStyle my-6 text-center */}
					<h1 className={classes.header.name}> { /* modern: header.nameStyle mb-2 text-3xl font-bold font-serif text-gray-800*/}
						{profile.personalDetails.firstName} {profile.personalDetails.lastName}
					</h1>
					<div className={classes.header.contact.container}> {/* modern: header.contactStyle flex justify-center items-center gap-2 mb-3 */}
						{profile.personalDetails.phoneNumber && (
							<div className={classes.header.contact.item}> {/* modern: header.contactItemStyle */}
								{/* <Phone size={14} /> */}
								<span>
									<a 
						...
						{profile.personalDetails.website && (
							<div className={classes.header.contact.item}> {/* modern: header.contactItemStyle */}
								<span className={classes.header.contact.separator} >|</span>{/*<Globe size={14} /> */}
								<span>
									<a 
										href={profile.personalDetails.website.startsWith("http") 
											? profile.personalDetails.website 
...
export function ResumeSectionTitle({ title, templateId }: ResumeSectionTitleProps) {
	const classes = getThemeClasses(templateId);

	return (
		<h2 className={classes.section.title}> { /* modern: section.titleStyle "mb-2 border-b border-black text-lg font-bold text-black"*/}
			{title}
		</h2>
  );
};
